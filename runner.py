...
for index, row in pd_PII.iterrows(): #for each product in the product information table
    ...
    GTIN=str(row['GTIN'])
    impactScoreID=str(GTIN) + str(timestamp)
    companyName=row['Company Name']
    productName=row['Product Name']
    ingredients=row['Ingredients'].split(',')
    lifestyles=row['Lifestyles'].split(',')
    nutrition={'sugars':row['Sugars per 100g'],
               'fat':row['Fat per 100g'],
               'saturates':row['Saturates per 100g'],
               'salt':row['Salt per 100g']}
    ...
    if all(value.strip() == '' for value in nutrition.values()):
    ...
    handle_xml.export_xml(root, filename)